...
from keras.layers import Conv1D, Dense, concatenate, Embedding, LSTM, Bidirectional, Dropout, Lambda
...
save_tensorboard_log_path = os.path.join(parent_dir_path, 'log', 'tensorboard_crnn')
...
# model
def textcrnn(max_sequence_length=max_reserved_vocab + 1, embedding_dim=embedding_dim, embedding_matrix=embedding_matrix,
    ...
    for i in range(len(kernel_sizes)):
        ...
        pool_output.append(c)
    pool_output[0] = Lambda(cut, arguments={'index': 2})(pool_output[0])
    pool_output[1] = Lambda(cut, arguments={'index': 1})(pool_output[1])
    x = concatenate([p for p in pool_output])
    x = Bidirectional(LSTM(units=cfg.rnn_filter_nums))(x)
    x = Dropout(0.5)(x)
...
parallel_model.compile(loss=cfg.loss_fine, optimizer=cfg.optimizer, metrics=cfg.metrics)
